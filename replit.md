...
**Setup status (2026-07-11):** Re-imported from GitHub into a fresh environment. Reinstalled dependencies, confirmed the Replit-managed Postgres DB and pushed the Drizzle schema, re-provisioned Replit-managed Clerk (dev keys), and registered/started all 6 artifact workflows. The API server auto-seeded the DB on first boot (6 languages, 6 courses, 45 lessons, 120 quiz questions). Full `pnpm run typecheck` passes across the workspace. `codecraft-mobile` (Expo) is still unregistered as an artifact and does not run in this environment.
