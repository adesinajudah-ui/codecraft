...
function CertificateCard({ name, completed, progress, xp, user }: {
  ...
  const handleDownload = () => {
    ...
    ctx.fillStyle = "#0f172a"; ctx.fillRect(0, 0, 900, 600);
    ctx.strokeStyle = "#22d3ee"; ctx.lineWidth = 6; ctx.strokeRect(30, 30, 840, 540);
    ctx.strokeStyle = "#0e7490"; ctx.lineWidth = 2; ctx.strokeRect(45, 45, 810, 510);
    ctx.fillStyle = "#22d3ee"; ctx.font = "bold 42px Georgia, serif"; ctx.textAlign = "center";
    ctx.fillText("Certificate of Completion", 450, 130);
    ctx.fillStyle = "#22d3ee"; ctx.fillRect(150, 150, 600, 2);
    ctx.fillStyle = "#94a3b8"; ctx.font = "20px Georgia, serif"; ctx.fillText("This certifies that", 450, 200);
    ctx.fillStyle = "#f1f5f9"; ctx.font = "bold 36px Georgia, serif"; ctx.fillText(user, 450, 255);
    ctx.fillStyle = "#94a3b8"; ctx.font = "20px Georgia, serif"; ctx.fillText("has successfully completed", 450, 305);
    ctx.fillStyle = "#22d3ee"; ctx.font = "bold 32px Georgia, serif"; ctx.fillText(`${name} Programming`, 450, 360);
    ctx.fillStyle = "#94a3b8"; ctx.font = "18px Georgia, serif";
    ctx.fillText(`CodeCraft · ${new Date().toLocaleDateString("en-US", { year: "numeric", month: "long", day: "numeric" })}`, 450, 400);
    ctx.fillStyle = "#22d3ee"; ctx.font = "bold 20px monospace"; ctx.fillText(`🏆 ${xp} XP Earned`, 450, 460);
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
      <Card className={`relative overflow-hidden border ${completed ? colors.border : "border-border"} transition-all`}>
        <div className={`absolute inset-0 bg-gradient-to-br ${colors.bg} pointer-events-none`} />
        <CardContent className="relative p-4">
          <div className="flex items-start justify-between mb-3">
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-xl bg-background/80 flex items-center justify-center border ${colors.border} flex-shrink-0`}>
                {completed
          ...
          {completed && (
            <div className="mb-3 p-2.5 rounded-lg bg-background/60 border border-border/50">
              <div className="flex gap-4 text-sm">
                <div>
                  <p className="text-xs text-muted-foreground">XP Earned</p>
                  <p className={`font-bold font-mono text-sm ${colors.text}`}>{xp} XP</p>
                </div>
                <div>
          ...
          <div className="flex gap-2">
            {completed ? (
              <>
                ...
                <Button size="sm" variant="outline" aria-label="Share certificate" className="gap-1.5 h-8 text-xs px-3">
  ...
  return (
    <div className="p-4 pb-8">
      <div className="mb-5">
        ...
        <p className="text-muted-foreground text-xs mt-0.5">Complete all lessons to earn a certificate.</p>
      </div>
          ...
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                You've earned <span className="font-bold text-primary">{earnedCount}</span> certificate{earnedCount > 1 ? "s" : ""}!
              </p>
            </div>
          </CardContent>
        </Card>
      ...
      <div className="space-y-3">
        {summaries && summaries.length > 0
          ...
          : ["HTML", "CSS", "JavaScript", "Python", "Java", "C"].map(name => (
              <CertificateCard key={name} name={name} completed={false} progress={0} xp={0} user={user?.fullName || "Developer"} />
            ))}
      </div>
    </div>
  );
